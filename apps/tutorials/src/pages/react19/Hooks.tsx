...
setCount(42); // If count is already 42, children won't re-render`}
      ...
      <LiveExample
        title="Try it — functional updater vs stale closure"
        height={380}
...
export default function Counter() {
  ...
  // 🔴 BAD: both reads see the same captured \`count\`.
  const bumpTwiceStale = () => {
    setCount(count + 1);
    setCount(count + 1);
  };

  // 🟢 GOOD: functional updater always gets the latest state.
  const bumpTwiceCorrect = () => {
    setCount((prev) => prev + 1);
    setCount((prev) => prev + 1);
  };
  ...
  return (
    <div style={{ fontFamily: 'system-ui', padding: 24, display: 'grid', gap: 12 }}>
      ...
      <button onClick={bumpTwiceStale}>Bump twice (stale)</button>
      <button onClick={bumpTwiceCorrect}>Bump twice (functional)</button>
