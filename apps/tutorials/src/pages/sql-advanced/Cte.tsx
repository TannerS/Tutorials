...
GROUP BY 1;`}
      ...
      <h2>Data-Modifying CTEs</h2>

      <CodeBlock language="sql" title="WITH ... AS (INSERT/UPDATE/DELETE ... RETURNING) — PostgreSQL Only" showLineNumbers={true}>
{`-- Move old orders to an archive table in a single statement, atomically.
-- Standard SQL CTEs are read-only; Postgres lets you chain writes with RETURNING.
WITH moved_rows AS (
  DELETE FROM orders
...
WHERE p.id = pu.product_id;`}
      ...
      <InfoBox variant="tip" title="Writable CTEs Chain Operations Atomically">
        <p>
          Because a data-modifying CTE and the statement that consumes it run in the same
          transaction implicitly, "delete from here, insert into there" becomes one atomic
          statement instead of two statements you'd otherwise have to wrap in{' '}
          <code>BEGIN...COMMIT</code> yourself. This is a distinctly Postgres-flavored pattern —
          most other engines don't support writable CTEs at all.
        </p>
      </InfoBox>
...
ORDER BY level, part_name;`}
      ...
      <InfoBox variant="danger" title="Infinite Recursion Protection">
        <p>
          ...
          until the DB kills it. PostgreSQL has no built-in recursion limit for CTEs by default.
        </p>
        <p>
          For graph data with cycles, track visited nodes: <code>WHERE e.id != ALL(visited_ids)</code>
          using an array accumulator, or use Postgres 14+'s built-in <code>CYCLE</code> clause (below).
        </p>
      </InfoBox>

      <CodeBlock language="sql" title="PostgreSQL 14+ CYCLE Clause" showLineNumbers={true}>
{`-- Postgres 14+ can detect and stop cycles natively, no manual array tracking needed
WITH RECURSIVE org_tree AS (
...
SELECT generate_series('2024-01-01'::date, '2024-12-31'::date, '1 day') AS d;`}
      ...
      <InfoBox variant="tip" title="Materialized vs Non-Materialized CTEs">
        <p>
          ...
          materialization (useful as an optimization fence to stop the planner pushing predicates
          in unhelpful ways), or <code>AS NOT MATERIALIZED</code> to force inlining even when
